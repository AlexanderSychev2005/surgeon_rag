...
def _fetch_full_text(doc: dict[str, Any]) -> tuple[str | None, str | None]:
...
def ingest_articles(
    client: QdrantClient, articles: list[dict[str, Any]]
    ...
    has_full_text = {
        doc["pmid"] for doc, section, _, _ in entries if section != "abstract"
    }
    ...
    points = [
        PointStruct(
            id=int(doc["pmid"])
            if section == "abstract"
            else str(uuid.uuid5(NAMESPACE, f"pmid:{doc['pmid']}:{section}")),
            vector=vector.tolist(),
            payload=_payload(
                doc,
                section=section,
                text=text,
                has_full_text=doc["pmid"] in has_full_text,
                source=source,
            ),
        )
