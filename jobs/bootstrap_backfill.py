...
def count_window(mindate: str, maxdate: str) -> int:
    _, _, count = esearch_history(
        COMBINED_PUBMED_QUERY, mindate=mindate, maxdate=maxdate
    )
    return count


def run_backfill(
    mindate: str, maxdate: str, limit: int | None = None, batch_size: int = BATCH_SIZE
) -> int:
    ...
    log.info(
        f"backfill window {mindate}..{maxdate}: {count} total matches, processing {total}"
    )
    ...
    for start in range(0, total, batch_size):
        ...
        log.info(
            f"backfill progress [{written}/{total}] +{n_points} points "
            f"({n_full}/{len(articles)} full text) — {rate:.1f} articles/s"
        )
    log.info(
        f"backfill done: wrote {written} articles from window {mindate}..{maxdate}"
    )
    return written


def run_backfill_medrxiv(mindate: str, maxdate: str, limit: int | None = None) -> int:
    client = ensure_collection()
    ...
    log.info(
        f"backfill medrxiv done: wrote {n_papers} preprints (+{n_points} points) from window {mindate}..{maxdate} — {rate:.1f} preprints/s"
    )
    return n_papers
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--medrxiv-only", action="store_true", help="only backfill medRxiv preprints"
    )
    ...
    if args.action == "count":
        ...
        if args.medrxiv_only:
            run_backfill_medrxiv(
                mindate=args.mindate, maxdate=args.maxdate, limit=args.limit
            )
        else:
